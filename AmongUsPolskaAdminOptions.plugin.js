...
module.exports = (() =>
{
	...
	const config =
	{
		...
		time:
		{
			...
			/* milliseconds */ busy_waiting_for_bot_timeout:    60e3,
			/* milliseconds */ unbusy_waiting_for_bot_timeout: 600e3,
		},
	};
	...
	let main = function()
	{
		...
		if (task_queue.length > 0)
		{
			...
			if (task.action == 'execute_dyno_command')
			{
				if (dyno_busy)
				{
					let dyno_tasks = new queue;
					let other_tasks = new queue;

					while (task_queue.length > 0)
					{
						let queued_task = task_queue.shift();
						if (queued_task.action == 'execute_dyno_command')
							dyno_tasks.push(queued_task);
						else
							other_tasks.push(queued_task);
					}

					while (other_tasks.length > 0)
						task_queue.push(other_tasks.shift());
					task_queue.push(task);
					while (dyno_tasks.length > 0)
						task_queue.push(dyno_tasks.shift());
				}
				else
				{
		...
		main_timer = setTimeout(main, delay);
	}
	...
	let tasks =
	{
		...
		execute_dyno_command: function(command)
		{
			...
			dyno_timer = setTimeout(function(){
				dyno_busy = false;
			}, config.time.busy_waiting_for_bot_timeout);
		},
	}
	...
	let warn = function(user_id, reason)
	{
		...
		}, config.time.unbusy_waiting_for_bot_timeout);
	...
	return (([Plugin, BDFDB]) =>
	{
		...
		return class AmongUsPolskaAdminOptions extends Plugin
		{
			...
			onStart()
			{
				main_timer = setTimeout(main, config.time.starting_delay);
			}
			...
			processMessage(e)
			{
				...
				if (channel_id == channels.komendy && author.bot && author.username == 'Dyno')
				{
					...
					if (m)
					{
						...
						if (user_warnings[warned_user_id])
						{
