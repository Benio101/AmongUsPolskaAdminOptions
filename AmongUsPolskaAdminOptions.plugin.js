 ...
 * @github https://github.com/Benio101/AmongUsPolskaAdminOptions
...
module.exports = (() =>
{
	...
	const config =
	{
		...
		time:
		{
			...
			/* milliseconds */ adding_reaction_timeout_min:       2e3,
			/* milliseconds */ adding_reaction_timeout_max:       4e3,
	...
	let main_loop = function()
	{
		...
		if (main_queue.length > 0)
		{
			...
			if (task.action == 'add_reaction')
			{
				...
				delay += config.time.adding_reaction_timeout_min + (Math.max(0, config.time.adding_reaction_timeout_max - config.time.adding_reaction_timeout_min) * Math.random());
			}
		}
...
// BdApi.findModuleByProps('setChannel', 'setServerDeaf', 'setServerMute')
// BdApi.findModuleByProps('fetchMessages').fetchMessages(BdApi.findModuleByProps('getChannel').getChannel('755646959015952476'))
